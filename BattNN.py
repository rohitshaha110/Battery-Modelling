...
def plot_current_and_voltage_twin(current,voltage,title=None):
    ...
    ax1.set_ylabel('Predicted Voltage(V)', color='#056db1')
    ...
    ax2.set_ylabel('Actual Voltage(V)', color='#fe0000')
...
def load_train_data():
    ...
    data = np.load(os.path.join('data/LabData','B1-cycle7-len135.npy'))
    current = data[0]
    voltage = data[1]
    I.append(current)
    V.append(voltage)
...
class BattNN(nn.Module):
    ...
    def train_one_epoch(self,print_per):
        ...
        self.rohit_lmse.append(l_1.item())

        #####
        if(self.iter%30==0):
            print(f'At Epoch: {self.iter}')
            name = f'yoyo_{self.iter}.pkl'
    ...
    def save_model(self,itr=1):
        if self.save_name is not None:
            torch.save(self.best_state, self.save_name)

    def load_model(self):
        checkpoint = torch.load(self.save_name,weights_only=True)
        self.load_state_dict(checkpoint['net'])
        self.optimizer.load_state_dict(checkpoint['optimizer'])
        self.iter = checkpoint['epoch'] + 1

    def load_model_2(self,name):
